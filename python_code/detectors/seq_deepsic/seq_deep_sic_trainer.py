...
from python_code.channel.channels_hyperparams import N_ANT, N_USER, MODULATION_NUM_MAPPING, CONSTELLATION_BITS
from python_code.channel.modulator import MODULATION_DICT
from python_code.detectors.seq_deepsic.seq_deep_sic_detector import SeqDeepSICDetector
from python_code.detectors.trainer import Trainer
from python_code.utils.config_singleton import Config
from python_code.utils.constants import HALF, ModulationType
from python_code.utils.trellis_utils import prob_to_QPSK_symbol, prob_to_BPSK_symbol, prob_to_EightPSK_symbol
...
class SeqDeepSICTrainer(Trainer):
    ...
    def _initialize_detector(self):
        self.detector = [[SeqDeepSICDetector().to(DEVICE) for _ in range(ITERATIONS)] for _ in
    ...
    def train_models(self, model: List[List[SeqDeepSICDetector]], i: int, tx_all: List[torch.Tensor],
    ...
    def forward(self, rx: torch.Tensor, h: torch.Tensor = None, tx_data: torch.Tensor = None) -> torch.Tensor:
        # detect and decode
        probs_vec = self._initialize_probs_for_infer()
        for i in range(ITERATIONS):
            probs_vec = self.calculate_posteriors(self.detector, i + 1, probs_vec, rx)
        confidence_word, confident_bits, detected_word = self.compute_output(probs_vec, tx_data, rx)
        return detected_word, (confident_bits, confidence_word)

    def compute_output(self, probs_vec, tx_data, rx):
        if conf.modulation_type == ModulationType.BPSK.name:
